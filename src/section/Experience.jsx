...
const experiences = [
  {
    ...
    duration: "2025",
    desc: "Built responsive web pages, optimized components, and strengthened practical React and JavaScript skills."
  },
  {
    role: "Full Stack Developer - ITInteract",
    company: "College Project",
    duration: "2025",
    desc: "Developed a full-stack system using React, Node.js, MongoDB. Implemented authentication and user dashboard."
  }
];
    ...
    return (
      <div className="relative flex flex-1 justify-center items-center min-w-0">
        ...
        >
          ...
          <p className="text-md text-gray-100 mb-3">{exp.company} | {exp.duration}</p>
          <p className="text-md text-gray-300 wrap-break-word">{exp.desc}</p>
        </motion.article>
      </div>
    )
  }
  return (
    <div className="relative flex items-start">
      ...
      >
        ...
        <p className="text-sm text-gray-300 wrap-break-word">{exp.desc}</p>
      </motion.article>
    </div>
  )
...
const Experience = () => {
  ...
  const SCENE_HEIGHT_HV = isMobile ? 160 * experiences.length : 120 * experiences.length;
  ...
  <div className="relative w-full max-w-md">
    ...
    {/* LINE */}
    <div className="absolute left-5 top-0 w-1 bg-white/20 rounded h-full"></div>
    <motion.div
      className="absolute left-5 top-0 w-1 bg-white rounded origin-top"
      style={{ height: lineSize }}
    />

    {/* CONTENT */}
    <div className="relative flex flex-col gap-10 ml-12 mt-6 pb-20">
      {experiences.map((exp, idx) => (
        <ExperienceItem
          key={idx}
          exp={exp}
