  ...
  return (
    <section id="about" className='min-h-screen w-full flex items-center justify-center relative bg-black text-white overflow-hidden'>
      ...
      <div className='relative z-10 max-w-6xl w-full mx-auto px-6 md:px-10 lg:px-12 py-20 flex flex-col gap-12'>
        <motion.div className="flex flex-col gap-8 md:flex-row items-center md:items-stretch"
          initial={{ opacity: 0, y: 24 }}
          whileInView={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
          viewport={{ once: true, amount: 0.4 }}
        >
          <motion.div className='relative w-[160px] h-[160px] md:w-[160px] md:h-[200px] rounded-2xl overflow-hidden shadow-2xl bg-gradient-to-br from-[#1cd8d2]/20 to-[#302b63]/20 border border-[#1cd8d2]/25'
          whileHover={{scale:1.02}}
          transition={{type: "spring", stiffness: 200, damping: 18}}
          >
            <img src={Profile} alt="profile-img" className='absolute inset-0' />
          </motion.div>

          <div className='flex-1 flex flex-col justify-center text-center md:text-left'>
            <h2 className='text-4xl sm:text-5xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-[#8e2de2] to-[#ff6bcb]'>
              Kaustubh Bankar
            </h2>
            ...
            <div className='mt-6 grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 max-w-xl'>
              {stats.map((elem, i) => (
                ...
                >
                  <div className='text-sm text-gray-400'>{elem.label}</div>
                  <div className='text-base font-semibold'>{elem.value}</div>
                </motion.div>
              ))}
            </div>
            ...
            <div className='mt-6 flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center md:justify-start'>
              ...
              <a href="#contact"  className="inline-flex items-center justify-center rounded-lg bg-transparent border border-white/30 text-white/80 font-semibold px-5 py-3 hover:bg-white hover:text-black transition">Get in Touch</a>
            </div>
